...
// Mega Darkrai ex, Mega Zeraora ex, Mega Chandelure ex, Mega Excadrill ex
// Hide 'n' Sneak archetype + support trainers
// ============================================
const ME5_PROXY_CARDS: Record<string, Partial<Card>> = {
        ...
        attacks: [
            {
                name: 'Dusk Raid',
                damage: 110,
                energyCost: ['darkness', 'darkness'],
                description: 'If your Benched Pokémon have any damage counters on them, this attack does 110 more damage.',
            },
            {
                name: 'Abyss Eye',
                damage: 0,
                energyCost: ['darkness', 'darkness', 'darkness'],
        ...
        attacks: [
            {
                ...
                description: 'This attack does 60 damage for each Lightning Energy attached to this Pokémon.',
            },
            {
                name: 'Zepto Turn',
                damage: 150,
        ...
        attacks: [
            {
                name: 'Undermine',
                damage: 90,
                energyCost: ['metal', 'metal'],
                description: 'Discard the top 2 cards of your opponent\'s deck.',
            },
            {
                name: 'Maximum Drilling',
                damage: 200,
                energyCost: ['metal', 'metal', 'metal'],
...
async function buildDeckHelper() {
    const [sv5, sv6, sv6pt5, sv7, sv8, sv8pt5, sv9, me1, me2, me2pt5, me3, me4, me5] = await Promise.all([
        fetchSet('sv5').catch(() => []),
        fetchSet('sv6').catch(() => []),
        fetchSet('sv6pt5').catch(() => []),
        fetchSet('sv7').catch(() => []),
...
// ============================================
// MEGA DARKRAI EX DECK — 2026 Standard (H-On)
// me5 (Pitch Black) — Darkness / Special Condition control
// 60 cards: 12 Pokémon / 36 Trainers / 12 Energy
// Strategy: Use status conditions to enable Abyss Eye 1-hit KO on any Pokémon
// Dusk Raid does 220 when benched Pokémon have damage counters
// ============================================
export async function createMegaDarkraiExDeck(): Promise<Card[]> {
    const { deck, addCard } = await buildDeckHelper();

    // Pokémon (12)
    addCard('Mega Darkrai ex', 3);          // me5 — Dusk Raid 220 / Abyss Eye instant KO on statused Pokémon
    addCard('Darkrai', 4);                  // me5 — Basic, Hypnoblast: Asleep (enables Abyss Eye)
    addCard('Mega Chandelure ex', 2);       // me5 — Binding Flame: +1 retreat cost; Phantom Maze 130+ damage
    addCard('Litwick', 2);                  // me5 — Basic for Chandelure line
    addCard('Lampent', 1);                  // me5 — Stage 1 bridge
    addCard('Meowth ex', 1);
    ...
    // Trainers — Items (22)
    addCard('Dark Order', 4);               // me5 — search Dark Pokémon or Darkness Energy
    addCard('Ultra Ball', 4);
    addCard('Abyss Scope', 3);              // me5 — peek hand + optional forced switch
    addCard('Rare Candy', 3);               // Litwick → Mega Chandelure ex (skip Lampent)
    addCard('Night Stretcher', 3);          // sv8 — recovery from discard
    addCard('Switch', 3);
    addCard('Super Rod', 2);
    addCard('Shadowy Cloak', 1);            // me5 — tool: no Weakness on attached Pokémon (ACE SPEC tier)
...
// ============================================
// HIDE 'N' SNEAK / NIGHT MARCH DECK — 2026 Standard (H-On)
// me5 (Pitch Black) — Ghost/Grass spread + burst
// 60 cards: 20 Pokémon / 30 Trainers / 10 Energy
// Strategy: Fill discard with Hide 'n' Sneak Pokémon,
//   Dhelmise hits for 170 with 4+ in discard (30+140)
//   Sinistcha Matcha Spin places 4 counters on ALL opponent Pokémon with 6+ in discard
//   Banette fuels the engine by searching any card via Puppet Pull
// ============================================
export async function createHideNSneakDeck(): Promise<Card[]> {
    const { deck, addCard } = await buildDeckHelper();

    // Pokémon (20) — maximize Hide 'n' Sneak count in discard
    addCard('Dhelmise', 3);                 // me5 — main attacker: 170 with 4+ HnS in discard
    addCard('Poltchageist', 4);             // me5 — Hide 'n' Sneak: discard fuel
    addCard('Sinistcha', 3);               // me5 — Hide 'n' Sneak: Matcha Spin 4 counters on all with 6+
    addCard('Shuppet', 4);                  // me5 — Hide 'n' Sneak: discard fuel
    addCard('Banette', 3);                  // me5 — Hide 'n' Sneak + Puppet Pull: search any card
    addCard('Fezandipiti ex', 2);           // sv6pt5 — Flip the Script: draw 3 after a KO
    addCard('Munkidori', 1);                // sv6 — Adrena-Brain: move damage counters
    addCard('Meowth ex', 1);
    ...
    // Trainers — Items (18)
    addCard('Ultra Ball', 4);               // discard + search: key for dumping HnS Pokémon
    addCard('Nest Ball', 4);                // place basics onto bench
    addCard('Night Stretcher', 3);          // sv8 — recover Pokémon from discard
    addCard('Switch', 3);
    addCard('Super Rod', 2);
    addCard('Counter Catcher', 2);          // sv6 — force active swap after a KO
    ...
    addCard('Grass Energy', 4);

    console.log(`[2026 Standard] Hide 'n' Sneak deck: ${deck.length} cards`);
    return shuffle(deck);
}
